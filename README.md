...
## 商品列表分页用例说明
- `testcase/ProductManager/getProductList.yaml` 中的商品列表接口数据包含三种分页场景：第 1 页、第二页（size=10）以及新增的第 3 页（size=5）。
- 新增的第三页用例验证：当 `page=3` 且 `size=5` 请求商品列表时，接口依旧返回 `error_code=0000`，确保分页参数变更不会影响接口正常返回。
